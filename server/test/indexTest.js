...
describe('iReporter', () => {
	...
	describe('single red flag', () => {
		it('should return single red-flag record', async () => {
			const red_flag = data[0];
			const response = await server.get(`/api/v1/red-flags/${red_flag.id}`);
			expect(red_flag).to.have.property('id').to.equal(red_flag.id);
	...
	describe('delete red-flag record', () => {
		it('should return 200 status indicating a successful delete', async () => {
			const red_flag = data[0];
			const response = await server.delete(`/api/v1/red-flags/${red_flag.id}`);
			expect(response.status).to.equal(200);
